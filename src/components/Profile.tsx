...
import {
  ...
  Lightbulb, Handshake, Plane, Star, Feather, Brain, Shield, Sparkles, User
} from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ onBack }) => {
  ...
  const contentData = {
    about: { content: ["Born in 2006, I'm a computer engineering student at NTU with a rather unconventional setup. I'm doing my degree while serving as a Digital Specialist in National Service. It's a four-year program where I alternate between studying at university and working on Singapore's digital infrastructure.", "Most of my free time is spent glued to my computer, tinkering with new projects and obsessively checking out the latest LLM models. I get genuinely excited about all things AI - whether it's the latest image generation breakthrough or voice synthesis that sounds eerily human. But what really gets me going is the bigger picture: superintelligence and what that means for humanity.", "I decided to build this site after reading about the \"date me docs\" phenomenon and the shift back to IRL dating. But instead of a plain Google Doc, I thought: why not build something that actually reflects who I am? Plus, I figured if someone's going to date an engineer, they should know what they're getting into from the start.", "Fair warning: I'm what you might call \"romantically inexperienced\" (okay fine, ultra virgin), but I'm optimistic about changing that status with the right person who appreciates both my ambition and my complete inability to not talk about AI."] },
    ...
    'venturecapital': { content: ["Lorem ipsum dolor sit amet, consectetur adipiscing elit."] },
  ...
  const getIcon = (category: string, item: string): React.ElementType => {
    const icons: { [key: string]: { [key: string]: React.ElementType } } = {
        about: { 
          'About': User 
        },
        interests: {
            'Superintelligence': BrainCircuit,
            'Space': Rocket,
            'Games': Gamepad2,
            'Startups': Lightbulb,
    ...
    return icons[category]?.[item] || Star;
  };

  const handleItemClick = (item: string) => {
    const key = item.toLowerCase().replace(/\s+/g, '') as keyof typeof contentData;
    if (contentData[key]) {
      setModalContent(contentData[key]);
    }
  };

  const renderSection = (title: string, items: string[], category: keyof typeof profileData) => (
    <section>
      <h3 className="text-2xl font-semibold mb-6">{title}</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item, index) => {
          const IconComponent = getIcon(category, item);
          return (
            <div key={index} onClick={() => handleItemClick(item)} className="block bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg p-4 transition-all duration-200 button-shadow hover:button-shadow-hover hover:scale-105 h-full cursor-pointer">
    ...
    <div className="min-h-screen w-full p-4 sm:p-6 md:p-8 text-white">
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-12">
          {renderSection("About Me", profileData.about, "about")}
          {renderSection("Interests", profileData.interests, "interests")}
          {renderSection("Values", profileData.values, "values")}
          {renderSection("Looking For", profileData.lookingFor, "lookingFor")}
          {renderSection("Story", profileData.story, "story")}
        </div>
